    ...
    try:
        ...
        for chat_id in MOVIE_UPDATE_CHANNELS:
            try:
                ...
                posted_to.append(str(chat_id))
            except Exception as e:
                logger.error(f"Failed to post to {chat_id}: {e}")
                failed_to.append(f"{chat_id} ({e})")

        if posted_to:
            status_text = f"✅ Post has been sent to {len(posted_to)} channel(s)."
            if failed_to:
                status_text += f"\n❌ Failed for: {', '.join(failed_to)}"
            await status_msg.edit(status_text)
            logger.info(f"Successfully posted '{session['movie_name']}' to {posted_to}.")
        else:
            await status_msg.edit(f"❌ Failed to post to any channel.\nErrors: {', '.join(failed_to)}")
