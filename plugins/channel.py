...
async def send_movie_update(bot, base_name):
    ...
    if not message_ids and movie_doc.get("message_id"):
        # Assuming the old message_id belongs to the first channel in MOVIE_UPDATE_CHANNELS
        if MOVIE_UPDATE_CHANNELS:
            message_ids[str(MOVIE_UPDATE_CHANNELS[0])] = movie_doc["message_id"]
...
async def update_movie_message(bot, base_name):
    try:
        ...
        if any(str(cid) not in message_ids for cid in MOVIE_UPDATE_CHANNELS):
            await send_movie_update(bot, base_name)
            # Re-fetch after update to get new message_ids
            movie_doc = await db.movie_updates.find_one({"_id": base_name})
            message_ids = movie_doc.get("message_ids", {})
        ...
        for chat_id_str, message_id in message_ids.items():
            ...
            try:
                ...
                pass
            except (MessageIdInvalid, FloodWait, Exception) as e:
                if isinstance(e, FloodWait):
                    await asyncio.sleep(e.value + 2)
                    # Retry once or just skip for now, let next schedule handle it
                    continue

                logger.warning(f"Failed to update message in {chat_id}: {e}")
                # If message is invalid, it might be deleted. Remove and try re-sending later.
                if isinstance(e, MessageIdInvalid):
                    updated_message_ids.pop(chat_id_str, None)
                    need_db_update = True

        if need_db_update:
            await db.movie_updates.update_one(
                {"_id": base_name},
                {"$set": {"message_ids": updated_message_ids}}
            )
            # Trigger re-send for missing ones
            await send_movie_update(bot, base_name)
